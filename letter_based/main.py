...
Do NOT reason or think before responding. Output the result immediately.
...
def checker(COMPRESSED:str, RESTORED:str)->str:
    response: ChatResponse = chat(model=CHECKER_MODEL, messages=[
        {'role': 'system', 'content': system_decode},
        {'role': 'user', 'content': f'the following is the COMPRESSED text: {COMPRESSED}'},
        {'role': 'user', 'content': f'the following is the RESTORED text: {RESTORED}'}
    ], think=False)
    return response['message']['content']
